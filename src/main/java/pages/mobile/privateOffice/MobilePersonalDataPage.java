...
public class MobilePersonalDataPage extends MainTestBase {
    ...
    @Step("Пользователь проверяет, что поле телефона не кликабельно и значение атрибута только на чтение")
    public void checkPhone() {
        getPhoneNumber().elementIsNotClickable();
        getPhoneNumber().checkAttribute("readonly", "true");
        logger.info("Пользователь проверяет, что поле телефона не кликабельно и значение атрибута только на чтение");
    }
